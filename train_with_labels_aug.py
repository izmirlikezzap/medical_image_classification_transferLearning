...
# Veri kümesi konumu
DATASET_ROOT = "/media/white-shark/backup_8tb_m2/Padchest/disease_classification/seperate/seperate_originals_aug"

# >>> EKLENDİ: Maskelerin kök klasörü
MASKS_ROOT = "/media/white-shark/backup_8tb_m2/Padchest/disease_classification/seperate/diseased_regions_aug"
...
class Config:
    def __init__(self, dataset_combination, device, classification_task):
        ...
        self.results_dir = f"aug_2class_wlabel_results_{classification_task}_{self.dataset_name}"
...
class XrayDataset(Dataset):
    ...
    def _infer_mask_path(self, image_file: Path, class_name: str):
        ...
        Sadece birebir isim eşleşirse maskeyi döndürür.
        Eğer maske yoksa -> None döner.
        """
        if self.masks_root is None:
            return None

        # Normal sınıfta maske zorunlu değil
        ...
        cand_same = class_ss_dir / Path(image_file).name
        if cand_same.exists():
            return str(cand_same)

        # Tam eşleşme yoksa maske yokmuş gibi davran
        return None
...
def train_single_model(model_name, model_factory, dataset_combination, device_ids, classification_task, max_retries=3):
    ...
    result_file = os.path.join(config.results_dir, f"aug_2class_wlabel_{model_name}_{classification_task}_results.json")
    partial_file = os.path.join(config.results_dir, f"aug_2class_wlabel_{model_name}_{classification_task}_partial_results.json")
    ...
    while attempt < max_retries:
        try:
            ...
            for epoch in range(config.num_epochs):
                ...
                if current_acc > best_acc:
                    ...
                    torch.save(model.module.state_dict(),
                               os.path.join(config.results_dir, f"aug_2class_wlabel_{model_name}_{classification_task}_best_model.pth"))
                else:
                    patience_counter += 1
            ...
            losses_path = os.path.join(config.results_dir, f"aug_{model_name}_{classification_task}_epoch_losses.csv")
            losses_df.to_csv(losses_path, index=False)

            cm_path = os.path.join(config.results_dir, f"aug_{model_name}_{classification_task}_confusion_matrix.csv")
...
def save_final_results(results, total_time):
    results_dir = Path("aug_final_results_xray_analysis_label_2class")
    ...
    with open(results_dir / "aug_2class_wlabel_xray_training_summary.json", 'w') as f:
    ...
    if results:
        ...
        df.to_csv(results_dir / "aug_2class_wlabel_xray_detailed_results.csv", index=False)
