...
class Config:
    def __init__(self, dataset_root, device, classification_task):
        self.images_root = dataset_root
        self.classification_task = classification_task
        self.results_dir = f"results_{classification_task}"
...
class XrayDataset(Dataset):
    def __init__(self, images_root, transform=None, class_names=None, masks_root=None):
        ...
        print(f"Dataset loaded: {len(self.samples)} samples from {self.images_root.name}")
...
class Trainer:
    ...
    def get_data_loaders(self, config):
        ...
        print(
            f"Train dataset ({config.classification_task}): {len(train_dataset)} samples | {train_counts}")
        print(
            f"Test dataset ({config.classification_task}): {len(test_dataset)} samples | {test_counts}")

        return train_loader, test_loader

    def train_epoch(self, model, loader, criterion, optimizer, device, class_names=None):
        model.train()
    ...
    def validate_epoch(self, model, loader, criterion, device, class_names=None):
        model.eval()
        total_loss, preds, targets = 0.0, [], []
        with torch.no_grad():
    ...
    def save_confusion_matrix(self, preds, targets, class_names, output_path):
        cm = confusion_matrix(targets, preds, labels=list(range(len(class_names))))
        cm_df = pd.DataFrame(cm, index=class_names, columns=class_names)
        cm_df.to_csv(output_path)
        print(f"Confusion matrix saved to {output_path}")

    def train_single_model(self, model_name, model_factory, dataset_root, device_ids, classification_task, max_retries=3):
        config = Config(dataset_root, device_ids[0], classification_task)
        result_file = os.path.join(config.results_dir, f"{model_name}_{classification_task}_results.json")
        partial_file = os.path.join(config.results_dir, f"{model_name}_{classification_task}_partial_results.json")
        ...
        while attempt < max_retries:
            try:
                ...
                train_loader, test_loader = self.get_data_loaders(config)
                ...
                for epoch in range(config.num_epochs):
                    train_loss, train_metrics = self.train_epoch(model, train_loader, criterion, optimizer, device_ids[0], config.class_names)
                    val_loss, val_metrics, val_preds, val_targets = self.validate_epoch(model, test_loader, criterion, device_ids[0], config.class_names)
                ...
                self.save_confusion_matrix(val_preds, val_targets, config.class_names, cm_path)
    ...
    def run_training(self):
        ...
        for task_name, task_classes in CLASSIFICATION_TASKS.items():
            ...
            for model_idx, (model_name, model_factory) in enumerate(model_configs.items()):
                ...
                result = self.train_single_model(model_name, model_factory, DATASET_ROOT, device_ids, task_name)
                if result:
    ...
    def save_final_results(self, results, total_time):
        results_dir = Path("final_results_xray_analysis")
        results_dir.mkdir(exist_ok=True)
        summary = {
            'experiment_info': {
                'purpose': 'X-ray classification',
        ...
        if results:
            ...
            df = df.sort_values(['Classification Task', 'Model Name'])
            ...
            for idx, row in top_models.iterrows():
                print(f"  {idx + 1:2}. {row['Model Name'][:15]:<15} ({row['Classification Task']}) "
                      f"- Acc: {row['Best Val Accuracy']:.4f}")
